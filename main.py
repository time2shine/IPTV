...
def get_latest_live_link(youtube, channel_id):
    ...
    try:
        request = youtube.search().list(
            part="id",  # only id for minimal quota usage
...
def main(api_key):
    ...
    for channel_id, metadata in channel_metadata.items():
        ...
        channel_logo = ''  # Skip logo fetching

        live_link = get_latest_live_link(youtube, channel_id)
        if not live_link:
            print(f"Skipping {channel_name}: no live video found")
            continue

        m3u8_link = get_stream_url(live_link)
        if not m3u8_link:
            print(f"Skipping {channel_name}: no stream link found")
            continue
    ...
    if output_data:
        ...
        print("No live streams found for any channels.")

if __name__ == "__main__":
    # Select API key based on current hour
    if 0 <= now.hour < 3:
        api_key = "AIzaSyBX_LlRNOxBzT5eAWzRiCWNjFS000uqsBQ"  # allmybooks
    elif 3 <= now.hour < 6:
        api_key = "AIzaSyCgJaZsz-tsyAaIJRLc5NRYQyC-vnTCwAI"  # rokonmagura
    elif 6 <= now.hour < 9:
        api_key = "AIzaSyA7ySJB3-FKUUWFpcT6PpCcF578936a8jQ"  # deshirambo11 (removed comma)
    elif 9 <= now.hour < 12:
        api_key = "AIzaSyA_58ry30FZ2qUQFNiJXJgNXJfCYj-0-9k"  # deshirambo12
    elif 12 <= now.hour < 15:
        api_key = "AIzaSyDYyv5PWLhOCRxAwMkiaH9VY_BXwMi3VzM"  # deshirambo13
    elif 15 <= now.hour < 18:
        api_key = "AIzaSyDJ5CkvzxGaJL99SdGqENypUVcm0nFaKEQ"  # deshirambo14
    elif 18 <= now.hour < 21:
        api_key = "AIzaSyDO8JaYU6HbD8PdypJhG-EkFi4nojq0hrE"  # deshirambo15
    else:  # 21 <= now.hour < 24
        api_key = "AIzaSyCxklIr0fXmsjmiwzoDfBBT0DxtMpWQS68"  # deshirambo16
